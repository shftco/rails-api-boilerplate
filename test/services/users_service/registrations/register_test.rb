    ...
    class RegisterTest < ActiveSupport::TestCase
      ...
      test 'should register user' do
        assert_difference(['User.count', 'Doorkeeper::AccessToken.count'], 1) do
          ...
          doorkeeper_token = service.success

          assert service.success?
          assert_equal params[:email], doorkeeper_token.resource_owner.email
          assert_equal doorkeeper_application.id, doorkeeper_token.application.id
        end
      end
